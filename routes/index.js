...
var checkLoggedInAdmin = require('./middleware/checkLoggedInAdmin.js');
...
// GET admin login
router.get('/admin', checkLoggedOut, function(req, res, next) {
    res.render('admin', {
        title: 'SmartGarden Admin | administrator login',
        title_slug: "admin-login"
    });
});
...
//ISSUE:passwords store as undefined in postgres right now through the form.
//so this doesn't authenticate by password yet.
router.post('/admin/submit/', checkLoggedOut, function(req, res) {
    var email = req.body.email;
    var pass = req.body.pass;

    db.getAdminUser(email, pass, function(user, success) {
        if (!success) {
            var msg = "incorrect login credentials";
            res.render('admin', {
                title: 'SmartGarden Admin | administrator login',
...
/* GET Admin Panel. */
router.get('/admin/panel', checkLoggedInAdmin, function(req, res, next) {
    res.render('panel', {
        title: 'SmartGarden Admin Panel | Innovating Gardening',
        title_slug: "panel",
        user: req.session.user.username
    });
});
